...
选号规则是「优先级 → 权重 → 累计请求数少的先用」，而累计请求数（管理页账号表里的「请求」列）**只增不减**。跑得久了，老账号的计数会远高于新导入的账号，「少的先用」就退化成「只有最闲的一两个在跑」。这不是固定路由，是历史计数把水位拉开了。

到「账号」页点**重置请求计数**，把全部账号的计数归零，四个账号就回到同一水位重新均衡。只归零用于选路的请求数，累计 Token、累计已用额度都不受影响。

### 同一个模型，国际号和国内号计费不一样

国内版与国际版分开计费（例如 `deepseek-v4.1-flash` 在国际版免费，在国内版扣额度）。到「模型配置」→**站点偏好**，给模型指定「国际版优先」或「国内版优先」，也可以只设一个默认值给没单独配置的模型用。

这是纯优先级：偏好那一边没有可用账号、或者账号都试过了，会自动退回另一边，不会让请求无账号可用。默认是「不区分」，行为和之前完全一样。
