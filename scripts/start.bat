...
REM Scripts live in scripts\; the project root is one level up.
cd /d "%~dp0.."
...
echo  [Stop] Ctrl+C
echo.
call "%BUDDY_CONDA_EXE%" run --no-capture-output -n buddy2api python -m buddy2api --port 8787 %*
goto end
...
echo  [Stop] Ctrl+C
echo.
.venv\Scripts\python.exe -m buddy2api --port 8787 %*
goto end
