...
#
# 用法：
#   ./scripts/run-macos.sh                 # 默认 workbuddy 通道，端口 8787
#   ./scripts/run-macos.sh --port 8788     # 换端口
#   CB_GATEWAY_PROVIDERS=workbuddy,traework ./scripts/run-macos.sh   # 自行覆盖通道
set -euo pipefail

# 脚本移到 scripts/ 后，项目根是上一级目录。
cd "$(dirname "$0")/.."
...
# 想彻底关掉这个保护（纯流式、不做停转重打）：CB_GATEWAY_TOOL_STALL_RETRY=0 ./scripts/run-macos.sh
...
exec .venv/bin/python -m buddy2api "$@"
